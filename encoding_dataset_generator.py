...
def run(size, audio_dir, batch_size):
    ...
    for split in ['training', 'validation', 'test']:
        ...
        # Make the arrays
        tracks_encoded = torch.zeros((len(Y), SIZE, 64), dtype=torch.int16) # 64 appears to be the length of an encoding
        ...
        for x, _ in tqdm.tqdm(loader):
            with torch.no_grad():
                ...
                es = torch.squeeze(encodings_quantized[0], 0).t()
                tokens = vqvae.encode(x, start_level=2, end_level=3, bs_chunks=x.shape[0])[0]
                tokens = torch.squeeze(tokens)

                tracks_encoded[idx:idx+es.shape[0], :tracks_encoded.shape[1], :] = es.short()
